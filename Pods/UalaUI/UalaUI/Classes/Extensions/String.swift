    ...
    func shortString() -> String {
        ...
        if !words.isEmpty {
            ...
            if !firstWord.isEmpty {
                displayString.append(String(firstWord.prefix(1)))
            }
        }
        ...
        if words.count > 1 {
            ...
            if words.count > 1, let last = lastWord, !last.isEmpty {
                displayString.append(String(last.prefix(1)))
            }
        }
...
public extension String {
    ...
    func formatBasicPhone() -> String {
        ...
        return "+549\(basicPhone.count > 10 ? String(basicPhone.suffix(10)) : basicPhone)"
    }
...
public extension String {
    ...
    func firstRange(of characters: Int) -> String {
        return String(self.prefix(characters))
    }
}
...
public extension NSMutableAttributedString {
    func replaceHTMLTag(tag: String, withAttributes attributes: [NSAttributedString.Key: Any]) {
        ...
        while true {
            ...
            self.addAttributes(attributes, range: NSRange(location: affectedLocation, length: closeTagRange.location - affectedLocation))
...
public extension String {
    ...
    @available(*, deprecated, message: "Function moved to UalaUtils")
...
public extension String {
    ...
    func withBoldText(text: String, font: UIFont? = nil) -> NSAttributedString {
        let myFont = font ?? UIFont.systemFont(ofSize: 14, weight: .regular)
        let fullString = NSMutableAttributedString(string: self, attributes: [NSAttributedString.Key.font: myFont])
        let boldFontAttribute: [NSAttributedString.Key: Any] = [NSAttributedString.Key.font: UIFont.boldSystemFont(ofSize: myFont.pointSize)]
...
extension String {
    ...
    func withBoldText(boldPartsOfString: [String], font: UIFont, boldFont: UIFont) -> NSAttributedString {
        ...
        for idx in 0 ..< boldPartsOfString.count {
            boldString.addAttributes(boldFontAttribute, range: (self as NSString).range(of: boldPartsOfString[idx] as String))
        }
        return boldString
    }
}
...
public extension String {
    func percentageInputFormatting() -> String {
        ...
        let regex = try? NSRegularExpression(pattern: "[^0-9]", options: .caseInsensitive)
        valueWithPrefix = regex?.stringByReplacingMatches(in: valueWithPrefix,
                                                          options: NSRegularExpression.MatchingOptions(rawValue: 0),
                                                          range: NSRange(location: 0, length: self.count),
                                                          withTemplate: "") ?? ""
