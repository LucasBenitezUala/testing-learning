...
public extension UIImageView {
    ...
    private func imageSnap(text: String?, color: UIColor, circular: Bool, textAttributes: [NSAttributedString.Key: Any]?) -> UIImage? {
        ...
        if let text = text {
            ...
            text.draw(
                in: CGRect(x: bounds.size.width/2 - textSize.width/2, y: bounds.size.height/2 - textSize.height/2, width: textSize.width, height: textSize.height),
...
public extension UIImageView {
    func downloadImage(urlString: String, contentMode mode: UIView.ContentMode = .scaleAspectFill, completion: ((Result<RetrieveImageResult, KingfisherError>) -> Void)? = nil) {
        guard let url = URL(string: urlString) else { return }
        downloadImage(url: url, contentMode: mode, completion: completion)
    }
    
    func downloadImage(url: URL, contentMode mode: UIView.ContentMode = .scaleAspectFill, completion: ((Result<RetrieveImageResult, KingfisherError>) -> Void)? = nil) {
        contentMode = mode
