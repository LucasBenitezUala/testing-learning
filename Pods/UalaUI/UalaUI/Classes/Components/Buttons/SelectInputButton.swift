...
public class SelectInputButton: UIButton {
    ...
    private var selectedOption: OptionSelect? {
        didSet {
            ...
            if let selectedOption = self.selectedOption {
                self.delegate?.selectInputButton(didChange: selectedOption)
            }
        }
    }
    ...
    func initComponent() {
        ...
        self.setTitleColor(UalaStyle.colors.blueGrey, for: .normal)
    ...
    func setIconToRight() {
        if imageView != nil {
            ...
            self.titleEdgeInsets = UIEdgeInsets(top: 0, left: 0, bottom: 0, right: (self.imageView?.frame.width) ?? 0)
        }
    }
    ...
    @objc func openPicker(sender: UIButton) {
        ...
        if let viewDis = self.viewDisplay {
            ...
            pickerViewController.handler = { _, result in
