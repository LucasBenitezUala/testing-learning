...
class MonthYearPickerViewController {
    ...
    func getPickerController(selectedDate: String? = nil) -> PickerViewController<String> {
        guard let selectedDate = selectedDate else {
            return PickerViewController<String>()
        }

        var selectedYear = !selectedDate.isEmpty ? selectedDate.toDateWith(format: "MM/yyyy").toStringWith(format: "yyyy") : ""
        
        configure(selectedYear: selectedYear)
        
        let selectedMonth = (!selectedDate.isEmpty ? selectedDate.toDateWith(format: "MM/yyyy").toStringWith(format: "MMMM") : months.last ?? "").capitalized
