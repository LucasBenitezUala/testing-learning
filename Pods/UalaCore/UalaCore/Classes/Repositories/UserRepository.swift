...
public class UserRepository {
    ...
    public func editUser(parameters: Parameters) -> Promise<Any?> {
        return Promise<Any?> { seal in
            APIManager.requestApi2(path: "users", method: .put, parameters: parameters, encoding: JSONEncoding.default).done({ (_) in
                seal.fulfill(nil)
            }).catch({ (error) in
                seal.reject(error)
            })
    ...
    public func validateCBU(id: String, value: String) -> Promise<Any?> {
        ...
        return Promise<Any?> { seal in
            APIManager.requestApi2(path: "cbus", method: .post, parameters: parameters, encoding: JSONEncoding.default).done({ (_) in
                    seal.fulfill(nil)
            }).catch({ (error) in
                seal.reject(error)
            })
    ...
    public func updateCBU(_ cbu: String) -> Promise<Any?> {
        ...
        return Promise<Any?> { seal in
            APIManager.requestApi2(path: "cbus", method: .put, parameters: parameters, encoding: JSONEncoding.default).done({ (_) in
                    seal.fulfill(nil)
            }).catch({ (error) in
                seal.reject(error)
            })
    ...
    public func updateUserInfo(info: [String: Any]) -> Promise<User> {
        return Promise<User> { seal in
            ...
            apiManager.requestApi2(path: "users", method: .put, parameters: parameters, encoding: JSONEncoding.default).done { json in
                guard let userResponse = try? JSONDecoder().decode(RemoteUser.User.self, from: json.rawData()) else {
