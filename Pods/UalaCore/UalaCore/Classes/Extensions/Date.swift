...
// TODO - Review the force unwraps in this extensions
...
public extension Date {
    ...
    /// Trim string after seconds
    /// - Parameter strDate: `String` value
    /// - Returns: `Date` object or `nil`
    static func fromShortedString(_ strDate: String) -> Date? {
        let formatter: DateFormatter = DateFormatter()
        formatter.dateFormat = "yyyy-MM-dd'T'HH:mm:ss"
        formatter.locale = Locale.init(identifier: "es_ES")
        formatter.timeZone = TimeZone(identifier: "UTC")
        
        let shortStr = strDate.substring(toIndex: 18)
        
        return formatter.date(from: shortStr)
    }
    ...
    static func fromDatePickerString(strDate: String) -> Date? {
        let formatter: DateFormatter = DateFormatter()
